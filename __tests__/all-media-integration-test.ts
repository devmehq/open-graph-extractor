...
describe('allMedia', function () {
  it('if more then one media tags are found, return the first one', async function () {
    ...
    expect(result.alIosUrl).toEqual('https://www.yelp.com/biz/boba-guys-san-francisco-4?utm_campaign=biz_details&utm_medium=organic&utm_source=apple');
    ...
    expect(result.favicon).toEqual('//s3-media2.fl.yelpcdn.com/assets/srv0/yelp_styleguide/118ff475a341/assets/img/logos/favicon.ico');
    ...
    expect(result.twitterAppUrliPhone).toEqual('yelp:///biz/18TtLS_JtiS2OH30FLqNrw?utm_campaign=default&utm_source=twitter-card');
    expect(result.twitterAppUrliPad).toEqual('yelp:///biz/18TtLS_JtiS2OH30FLqNrw?utm_campaign=default&utm_source=twitter-card');
    expect(result.twitterAppUrlGooglePlay).toEqual('intent://yelp.com/biz/18TtLS_JtiS2OH30FLqNrw?utm_source=twitter-card#Intent;scheme=http;package=com.yelp.android;end;');
